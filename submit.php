...
?>
        ...
        <td><?php echo $dept; ?></td>
        <td><?php echo $pp; ?></td>
       <td>  <?php $status=""; $N="N";if($row['status']==$N) {  ?> <input id="btn1" type="submit"  onchange="return val(this)" formaction="confirm.php" value="Submit" name="previous"></td> 
          <?php } else { echo "Submit"; $status="Submitted"; } ?> <td>  <font size="5"  face="Comic sans MS"  color="red"> <p> <?php  if($status=="Submitted") { echo "Submitted on ".$row['status']; } else echo "Pending";
       $_SESSION['msg']="";?> </p></font></td>
        
        <td> <?php $status=""; $N="N";if($row['status']!=$N) {  ?> <input id="btn1" type="submit"   formaction="gpdf.php" value="Get .pdf" name="previous">
