...
if($row['status']!="N") {
     if($row['post'] ==4)
{
      $pp="Assistant Professor(On Contract)PB-4 with Grade pay of Rs 9000";
    ...
    $ap11=$row['appid'];
//	echo $ap11;
 $sql1="SELECT * FROM newrules4src  where appn='$ap11' ";
//echo $sql1;
...
<td> <?php echo $inc; $inc=$inc+1;?></td>
   ...
   <?php $sql1 =  "SELECT * FROM eq4page WHERE appn='$vapnid'";
...
<td> <input type="hidden" name="usercredit" value="   <?php echo $scrrow1['credit1']; ?> " readonly/> <?php echo $scrrow1['credit1']; ?>  </td>
       ...
       <td> <?php if($xxxx==1) {echo $vcredit1;} else {echo "Credits Not Verified";} ?> </td> <td> 
             <?php  $status=""; $N="Y";if($srcstatus!=$N) {  ?>    <input style="  font-size: 10px;"  id="btn4"  type="submit" formaction="creditverify4.php"  value="Verify Credits" name="edit"></label>
...
if($row['status']!="N") {
          ...
          if($row['post'] ==3)  
{   $pp="Assistant Professor PB-5 with Grade pay of Rs.10500";
    ...
    if($row['dept'] =="AMH") $dept="Applied Mechanics and Hydraulics";
     ...
     $ap11=$row['appid'];
//	echo $ap11;
 $sql1="SELECT * FROM newrules5src  where appn='$ap11' ";
//echo $sql1;
 $result_set1=mysqli_query($link,$sql1);
$row1=$result_set1->fetch_array();
  $srcstatus= $row1['status'];
...
<td> <?php echo $inc; $inc=$inc+1;?></td>
            <td><input type="radio" name="viewsrc" value="<?php echo $row['appid']; $vapnid=$row['appid'] ?>" required/> <?php echo $row['appid']; ?> </td>
       <?php $sql1 =  "SELECT * FROM eq5page WHERE appn='$vapnid'";
$result11 =  mysqli_query($link, $sql1);
...
<td><input type="hidden" name="usercredit" value="<?php echo $scrrow1['credit1']; ?> " readonly/> <?php echo $scrrow1['credit1']; ?> </td>
 ...
 <td> <?php if($xxxx==1) {echo $vcredit1;} else {echo "Credits Not Verified";} ?> </td>
          <td> 
             <?php $status=""; $N="Y";if($srcstatus!=$N) {  ?>    <input  id="btn4"  type="submit" formaction="creditverify5.php"  value="Verify Credits" name="edit"></label>
                  <?php } else { echo "Verified";  } ?>  </font>   </td>    
